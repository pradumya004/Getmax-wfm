...
export const calculatePerformanceMetrics = async (employeeId, period = TIME_PERIODS.CURRENT_MONTH, customTargets = null) => {
    try {
        ...
        const claimTasks = await ClaimTasks.find({
...
const calculateProductivityMetrics = async (claimTasks, dateRange, targets) => {
...
const calculateQualityMetrics = async (claimTasks, targets) => {
    const completedTasks = claimTasks.filter(task => task.status === TASK_STATUS.COMPLETED);
...
const calculateEfficiencyMetrics = async (claimTasks, targets) => {
    const completedTasks = claimTasks.filter(task => task.status === TASK_STATUS.COMPLETED);
...
const calculateSLAMetrics = async (employeeId, dateRange, targets) => {
    try {
...
const calculateVolumeMetrics = async (claimTasks, targets) => {
...
const calculateAccuracyMetrics = async (claimTasks, targets) => {
...
const calculateTimelinessMetrics = async (claimTasks, targets) => {
...
const calculateOverallPerformanceScore = (metrics, weights) => {
    const overallScore =
